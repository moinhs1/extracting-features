# Phase 4: Layer 2 Therapeutic Class Indicators Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Generate 53 therapeutic class binary indicators for each patient-timewindow, using ingredient mappings from Phase 3 and class definitions from therapeutic_classes.yaml.

**Architecture:** Load mapped medications from silver, match ingredient names to 53 therapeutic classes, aggregate per patient per time window (baseline/acute/subacute/recovery), output gold parquet with class indicators, counts, and first-occurrence times.
...
## Task 1: Create Therapeutic Class Builder Tests

**Files:**
- Create: `/home/moin/TDA_11_25/module_04_medications/tests/test_class_indicator_builder.py`
...
```bash
cd /home/moin/TDA_11_25 && PYTHONPATH=module_04_medications:$PYTHONPATH pytest module_04_medications/tests/test_class_indicator_builder.py -v
```
...
```bash
git add module_04_medications/tests/test_class_indicator_builder.py
git commit -m "test(module4): add therapeutic class indicator tests

Add failing tests for:
- Ingredient to class mapping
- Dose-based therapeutic/prophylactic classification
...
## Task 2: Create Transformers Package

**Files:**
- Create: `/home/moin/TDA_11_25/module_04_medications/transformers/__init__.py`

**Step 1: Create transformers package init**

```python
# /home/moin/TDA_11_25/module_04_medications/transformers/__init__.py
"""
Module 4 Transformers
=====================

Transform silver medication records into gold analysis-ready features.
...
```bash
git add module_04_medications/transformers/__init__.py
git commit -m "chore(module4): create transformers package
...
## Task 3: Implement Class Indicator Builder

**Files:**
- Create: `/home/moin/TDA_11_25/module_04_medications/transformers/class_indicator_builder.py`

**Step 1: Create class indicator builder**
...
```bash
git add module_04_medications/transformers/class_indicator_builder.py
git commit -m "feat(module4): implement therapeutic class indicator builder
...
## Task 4: Run Test Mode Class Building

**Files:**
- Run: Class indicator builder in test mode

**Step 1: Run test mode**

```bash
cd /home/moin/TDA_11_25 && PYTHONPATH=module_04_medications:$PYTHONPATH python module_04_medications/transformers/class_indicator_builder.py --test
```
...
import pandas as pd
df = pd.read_parquet('module_04_medications/data/gold/therapeutic_classes/class_indicators_test.parquet')
print(f'Shape: {df.shape}')
print(f'Columns (first 20): {list(df.columns)[:20]}')
print(f'\\nPatients: {df[\"empi\"].nunique()}')
print(f'\\nTime windows:')
...
## Task 5: Run Full Class Building

**Files:**
- Run: Class indicator builder in full mode

**Step 1: Run full building**

```bash
cd /home/moin/TDA_11_25 && PYTHONPATH=module_04_medications:$PYTHONPATH python module_04_medications/transformers/class_indicator_builder.py 2>&1 | tee module_04_medications/class_building.log
```
...
df = pd.read_parquet('module_04_medications/data/gold/therapeutic_classes/class_indicators.parquet')

print('='*60)
print('Layer 2 Validation')
print('='*60)

print(f'\\nShape: {df.shape}')
print(f'Patients: {df[\"empi\"].nunique()}')

# Check anticoagulant coverage in acute window
acute = df[df['time_window'] == 'acute']
any_anticoag = (
    acute['ac_ufh_ther'] | acute['ac_ufh_proph'] |
    acute['ac_lmwh_ther'] | acute['ac_lmwh_proph'] |
    acute['ac_xa_inhibitor'] | acute['ac_vka'] |
    acute['ac_dti'] | acute['ac_fondaparinux']
)
anticoag_rate = any_anticoag.sum() / len(acute) * 100

print(f'\\nAnticoagulant in acute window: {anticoag_rate:.1f}%')
print(f'Target: ≥90%')
print(f'Status: {\"PASS ✓\" if anticoag_rate >= 90 else \"CHECK DATA\"}')
"
```
...
```bash
git add module_04_medications/class_building.log
git commit -m "chore(module4): Layer 2 class indicators built

Output: gold/therapeutic_classes/class_indicators.parquet
Anticoagulant coverage: XX.X%
...
- ✅ `class_indicator_builder.py` exists in transformers/
- ✅ All tests pass
- ✅ `class_indicators.parquet` created in gold/therapeutic_classes/
- ✅ 53 class columns present
- ✅ 4 time windows present (baseline, acute, subacute, recovery)
- ✅ Anticoagulant coverage in acute window ≥90%
- ✅ All cohort patients represented
...
|------|-------------|--------|
| 1 | Class builder tests | test_class_indicator_builder.py |
| 2 | Create transformers package | __init__.py |
| 3 | Implement class builder | class_indicator_builder.py |
| 4 | Test mode building | class_indicators_test.parquet |
| 5 | Full building | class_indicators.parquet |
