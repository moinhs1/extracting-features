...
def analyze_lab_coverage(features_file):
    ...
    # Extract test names (everything before first underscore followed by phase name)
    test_names = set()
    ...
    for idx, row in df.iterrows():
        ...
        for test in test_names:
            # Check if patient has ANY measurement for this test across all phases
            for phase in phases:
                count_col = f"{test}_{phase}_count"
                if count_col in df.columns:
                    if pd.notna(row[count_col]) and row[count_col] > 0:
                        has_lab = True
                        break
            if has_lab:
                break
    ...
    for test in test_names:
        # Count patients with at least one measurement for this test
    ...
    print("="*80)
    print()
    print(f"{'Rank':<6} {'Lab Test':<45} {'Patients':<12} {'Coverage':<12} {'Measurements':<15} {'Avg/Patient':<12}")
    print("-" * 110)
    ...
    for rank, (idx, row) in enumerate(coverage_df.head(20).iterrows(), 1):
        ...
        print(f"{rank:<6} {test_name:<45} {patient_count:<12} {coverage_pct:>6.1f}%     {total_measurements:<15} {avg_measurements:>6.1f}")
    ...
    if len(full_coverage) > 0:
        ...
        for idx, row in full_coverage.iterrows():
            print(f"  - {row['test_name']}: {int(row['total_measurements'])} measurements")
    ...
    if len(rare_tests) > 0:
        ...
        print("="*80)
        for idx, row in rare_tests.iterrows():
    ...
    output_file = features_file.parent / f"{features_file.stem}_coverage_report.csv"
